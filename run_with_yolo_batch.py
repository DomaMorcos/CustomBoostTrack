...
from detectors import YoloDetectorBatch
"""
...
def main():
    ...
    # Initialize tracker and results holders
    tracker = None
    results = {}
    frame_count = 0
    total_time = 0
    ...
    # Create detector
    det = YoloDetectorBatch(args.yolo_path)
    ...
    for (img, np_img), _, info, _ in data_loader:
        ...
        # Initialize new video results
        if video_name not in results:
            results[video_name] = []
            
        # Initialize new tracker for new video
        if frame_id == 1:
            ...
            if batch_frames:
                process_batch(det, tracker, batch_frames, batch_info, results, frame_count, total_time)
                batch_frames = []
                batch_info = []
        
        # Collect frames for batch processing
        batch_frames.append((img, np_img))
        batch_info.append((frame_id, video_name))
        
        # Process batch when it reaches batch_size
        if len(batch_frames) >= batch_size:
            total_time_delta, frame_count_delta = process_batch(det, tracker, batch_frames, batch_info, results, frame_count, total_time)
            total_time += total_time_delta
            frame_count += frame_count_delta
            batch_frames = []
            batch_info = []
...
def process_batch(detector, tracker, batch_frames, batch_info, results, frame_count, total_time):
    """Process a batch of frames with the detector and tracker using batch inference"""
    import time
    import torch
    import utils
    ...
    # Extract numpy images for batch detection
    np_images = [np_img for _, np_img in batch_frames]
    
    # Run batch inference on all images at once
    with torch.no_grad():
        batch_preds = detector.batch_inference(np_images)
    
    # Process each result with the tracker
    for i, ((img, np_img), (frame_id, video_name), pred) in enumerate(zip(batch_frames, batch_info, batch_preds)):
        ...
        if pred is None or len(pred) == 0:
            results[video_name].append((frame_id, [], [], []))
            continue
    ...
    print(f"Batch processed: {frame_count_delta} frames in {time_delta:.3f}s, {frame_count_delta/time_delta:.2f} FPS")
        
    return time_delta, frame_count_delta
